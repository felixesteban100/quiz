    ...
    const answers = answersArr.map((current, index) => {
        ...
        if(props.checked){
            if(current.option===true){
                return (
                    <div className='bg-green-700 h-full text-center p-2 rounded-md self-center text-white' key={index}>
                        <p>{answerfixed}</p>
                    </div>
                )
            }
                ...
                return (
                    <div className='bg-red-700 h-full text-center p-2 rounded-md self-center text-white' key={index}>
                        <p>{answerfixed}</p>
                    </div>
                )
            }
            ...
            return (
                <div className='bg-gray-700 h-full text-center p-2 rounded-md self-center text-gray-400' key={index}>
                    <p>{answerfixed}</p>
                </div>
            )
        }
        return (
            <div className={current.selected ? 'bg-blue-700 hover:bg-blue-500 text-center max-w-sm h-full p-2 rounded-md self-center cursor-pointer' : 'bg-black text-white hover:text-white hover:bg-gray-700 text-center max-w-sm h-full p-2 rounded-md self-center cursor-pointer'} onClick={() => props.select(props.id, index)} key={index}>
                <p>{answerfixed}</p>
            </div>
        )
